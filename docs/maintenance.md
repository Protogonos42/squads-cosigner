...
## 2026-08-28 — sixth pass: a member paying themselves

Catalogue source this time: the BonkDAO treasury drain (2026-07-06, Realms/spl-governance, ≈$20M). The vote-buying half does not map to Squads (membership is fixed, not token-weighted), but the payload does: BIP #76 was a "governance renewal" whose single operative clause was a token transfer to the proposer's own wallet, and it passed on a 2.9 % turnout. Checked against `src/rules.ts`: under an allowlist posture the shape is already `REFUSED_COUNTERPARTY`; under a deny-list posture (empty `allowDestinations`, which the README documents as a valid configuration) a transfer to a member's own key passed like any stranger's. Now `checkWithSimulation` fetches the multisig for every VaultTransaction (previously only ConfigTransactions) and passes its member keys to `evaluateVaultMessage` (`EvaluateOptions.members`); `destinationAllowed` refuses a member destination that is not explicitly allowlisted as `REFUSED_THEFT_SHAPED` (`destinations.member`). Fetch failure skips the check rather than refusing, because a static-only run (`--static`, fixtures) has no members either and must keep its existing verdicts. One test, four cases. Default rules unchanged; `rules.example.json` lists the author's float key on purpose, so proposals 1 and 4 still approve. 61 tests, 60 pass, 1 skipped.
