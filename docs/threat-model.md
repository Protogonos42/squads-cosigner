...
| Proposes a transfer to **their own member key** while you run a deny-list posture (empty `allowDestinations`) — the BonkDAO shape (July 2026: BIP #76 was a "governance renewal" whose only clause moved 4.43T BONK to the proposer's wallet) | `check` fetches the multisig's member list; a lamport or token destination that is a member key and not explicitly in `allowDestinations` is refused (`destinations.member`). Listing the key on purpose keeps it allowed — the author's own float key is a member and is listed. Member list unfetchable → this check is skipped and `allowDestinations` alone governs, so an allowlist posture is still the stronger one | `REFUSED_THEFT_SHAPED` |
